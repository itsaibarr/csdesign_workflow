...
export default async function Page() {
    ...
    const totalTimeSaved = artifacts.reduce((acc, art) => acc + (art.reflection?.timeSavedMinutes || 0), 0);
    ...
    const completedNodesCount = user?.UserNodeProgress.filter(p => p.status === 'COMPLETED').length || 0;
                                    ...
                                    <div key={art.id} className="glass-card p-6 rounded-[2rem] border border-white/5 group hover:border-primary/30 transition-all flex flex-col">
                                        <div className="flex justify-between items-start mb-6">
                                            ...
                                            <span className={cn(
                                                ...
                                                art.status === 'VALIDATED' ? "bg-green-500/20 text-green-500 border-green-500/30" : "bg-primary/20 text-primary border-primary/30"
                                            )}>
                                                {art.status}
                                            </span>
                                        </div>
